<?php 
    ...
    $fetch=$select->fetch(); 
	$select_about=$con->prepare('SELECT * FROM aboutmech WHERE id=1');
    $select_about->execute();
	$fetch_about= $select_about->fetch();
	$select_message=$con->prepare('SELECT * FROM mechmessage WHERE id=1');
    $select_message->execute();
	$fetch_message= $select_message->fetch();
	$select_table=$con->prepare('SELECT * FROM mechtable ORDER BY id DESC');
    ...
    if($fetch['staute']==0){ ?>
		...
		<html>
			<head>
				<link rel='shortcut icon' href="../layout/pattern/close.png">
				 <title><?php $fetch['title']; ?> | قسم هندسة الميكاترونيكس</title>
	   ...
	   <?php 
		...
		$department='active';
		$title=$fetch['title'].' | هندسة الميكاترونيكس ';
                ...
                <div class="depertment_pre section-full bg-white content-inner-2" style="background-image:url(../layout/pattern/pt1.png);">
                                        <div class="container">
                                          <div class="section-head text-center">
                                                <h3 class="title">قسم هندسة الميكاترونيكس</h3>
                                        </div>
                                        ...
                                        <img src="../uploads/mech/<?php echo $fetch_about['img']; ?>" class="img" alt=""/>
											...
											<div class="accordion faq-box m-b10" id="accordion<?php echo $fetch_tab['id']; ?>">
												<div class="card">
													...
													data-parent="#accordion<?php echo $fetch_tab['id']; ?>">
														<div class="card-body"><a href="../uploads/mech/<?php echo $fetch_tab['img']; ?>" 
																   download="../uploads/mech/<?php echo $fetch_tab['img']; ?>"> تحميل الملف المطلوب </a>
														</div>
