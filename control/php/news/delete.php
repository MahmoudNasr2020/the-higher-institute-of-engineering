...
if(!isset($_SESSION['email'])){
  ...
  exit();
   ...
   if ($_SERVER['REQUEST_METHOD']=='POST') {
       ...
       $select_file_img=$con->prepare('SELECT * FROM news WHERE id=?');
       ...
       if($file_image_unlink != $img_file_data){
        unlink("../../../uploads/news/".$file_image_unlink);
       }
      
       $delete=$con->prepare('DELETE FROM news WHERE id=?');
           ...
           foreach ($fetch as $fet=>$fe) {
               $data['content'].='
                       ...
                       <tr role="row">
                           ...
                           <td> '.$fe['date'].'</td>
                           <td>
                                    <img src="../../../uploads/news/'.$fe['img'].'" class="mr-2" alt="image"> 
                           </td>
                           <td>
