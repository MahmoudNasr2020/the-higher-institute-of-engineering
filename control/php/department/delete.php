...
if(!isset($_SESSION['email'])){
 ...
 if ($_SERVER['REQUEST_METHOD']=='POST') {
     ...
     $select_file_img=$con->prepare('SELECT * FROM civiltable WHERE id=?');
     $select_file_img->execute(array($id));
     $fetch_file_img=$select_file_img->fetch();
     $file_image_unlink=$fetch_file_img['img'];
     unlink("../../../uploads/civil/".$file_image_unlink);
     $delete=$con->prepare('DELETE FROM civiltable WHERE id=?');
     $delete->execute(array($id));
     $select=$con->prepare('SELECT * FROM civiltable ORDER BY id DESC');
     $select->execute();
