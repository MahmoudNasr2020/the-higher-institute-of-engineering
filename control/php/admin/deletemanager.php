...
if ($_SERVER['REQUEST_METHOD']=='POST') { 
    ...
    $select_file_img=$con->prepare('SELECT * FROM admin WHERE id=?');
    ...
    if($file_image_unlink != $img_file_data){
     unlink("../../../uploads/admin/".$file_image_unlink);
    }
    $delete=$con->prepare('DELETE FROM admin WHERE id=?');
    $delete->execute(array($id));
    $select=$con->prepare('SELECT * FROM admin ORDER BY id DESC');
    $select->execute();
        ...
        $data['content']='<table class="table">
                        ...
                        <tbody>';
                            ...
                            foreach($fetchglobal as $fe=>$fetch){ 
                                $data['content'].='<tr>
                            <td>
                              <img src="../../../uploads/admin/'.$fetch['img'].'" 
