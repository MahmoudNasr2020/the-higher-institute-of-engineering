...
if(!isset($_SESSION['email'])){
 ...
 if ($_SERVER['REQUEST_METHOD']=='POST') { 
     ...
     $select_file_img=$con->prepare('SELECT * FROM techass WHERE id=?');
     ...
     $delete=$con->prepare('DELETE FROM techass WHERE id=?');
     $delete->execute(array($id));
     $select=$con->prepare('SELECT * FROM techass ORDER BY id DESC');
     $select->execute();
