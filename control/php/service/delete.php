...
if(!isset($_SESSION['email'])){
 ...
 if ($_SERVER['REQUEST_METHOD']=='POST') { 
     ...
     $select_file_img=$con->prepare('SELECT * FROM service WHERE id=?');
     $select_file_img->execute(array($id));
     $fetch_file_img=$select_file_img->fetch();
     $file_image_unlink=$fetch_file_img['img'];
     unlink("../../../uploads/serves/".$file_image_unlink);
     $delete=$con->prepare('DELETE FROM service WHERE id=?');
         ...
         $data['content']='<table class="table">
                         ...
                         <tbody>';
                             ...
                             foreach($fetchglobal as $fe=>$fetch){ 
                                 $data['content'].='<tr>
                             <td>
                               <img src="../../../uploads/serves/'.$fetch['img'].'" 
